...
Sometimes a stale file manager window lets you drop files into the vault folder while it is locked. Those files are **not encrypted**. On the next unlock, Omavault moves them to a `recovered` area and the panel offers two choices:
